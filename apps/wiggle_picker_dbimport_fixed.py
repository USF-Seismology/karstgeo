...
# Database lookup is intentionally optional and project-specific.
# Prefer segy_tools.db when installed, but also support a local db.py placed
# beside this script (common when running the picker as a standalone program).
_st_db = None
_db_import_error = None
try:
    import segy_tools.db as _st_db
except Exception as exc:
    _db_import_error = exc
    try:
        import db as _st_db  # local/project helper module on PYTHONPATH/CWD
    except Exception as exc2:
        _db_import_error = exc2
        try:
            # Last-resort standalone-script fallback: load ./db.py from the
            # same directory as this wiggle_picker.py file, without requiring
            # package installation or PYTHONPATH edits.
            import importlib.util
            _db_path = Path(__file__).resolve().parent / "db.py"
            if not _db_path.exists():
                # Helpful during staged refactors where the generated file may
                # still be called db_fixed.py before being renamed.
                _db_path = Path(__file__).resolve().parent / "db_fixed.py"
            if _db_path.exists():
                _spec = importlib.util.spec_from_file_location("_wiggle_picker_db", _db_path)
                _mod = importlib.util.module_from_spec(_spec)
                assert _spec is not None and _spec.loader is not None
                _spec.loader.exec_module(_mod)
                _st_db = _mod
        except Exception as exc3:
            _db_import_error = exc3
            _st_db = None

...
try:
    # Generic SEG-Y/SU header and writer functionality lives in segy_tools.
    # specfem_tools.io only decides how SPECFEM outputs become ObsPy Streams.
    from segy_tools.io import read_su_file, write_segy, make_trace_header
...
def _attach_standard_trace_header(
    ...
    *,
    ...
    if make_trace_header is None:
        raise ImportError("segy_tools.io.make_trace_header is not available.")
...
def write_model_products(
    ...
    verbose: bool = True,
) -> tuple[Stream, dict, Path | None, Path | None]:
    """Load one SPECFEM model gather and optionally write SEG-Y.

    This function is intentionally SPECFEM-focused.  Plotting has been moved to
    ``segy_tools.plotting`` / ``segy_tools.gather`` and is not performed here.
    The legacy plotting arguments are still accepted so older wrappers do not
    fail immediately, but ``make_plot=True`` now emits a warning and is ignored.
    ...
    write_segy_file
        Write ``<model>_<component>.segy`` under ``config.segy_out_dir`` using
        ``segy_tools.io.write_segy``.
    make_plot, tmin, tmax, scale, normalize
        Deprecated plotting options retained for API compatibility only.

    Returns
    -------
    stream, result, segy_file, fig_file
        Processed stream, raw loader result, SEG-Y path if written/requested, and
        ``None`` for the deprecated figure path.
    """
    ...
    if make_plot:
        warnings.warn(
            "specfem_tools.io.write_model_products(make_plot=True) is deprecated "
            "and ignored. Use segy_tools.plotting/gather plotting functions after "
            "writing or loading the SEG-Y file.",
            DeprecationWarning,
            stacklevel=2,
        )
    ...
    return st, result, segy_file if write_segy_file else None, fig_file
...
def batch_write_model_products(
    ...
    **kwargs,
    ...
    """Process multiple SPECFEM model directories into SEG-Y products.

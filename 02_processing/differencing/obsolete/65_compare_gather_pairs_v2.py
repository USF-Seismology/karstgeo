...
def plot_three_panel(
    ...
    *,
    ...
    txt = (
        "Difference strength\n"
        f"peak right / peak left = {fmt_pct(metrics.get('maxdiff_over_max_reference_pct', np.nan))}\n"
        f"peak right / peak center = {fmt_pct(metrics.get('maxdiff_over_max_comparison_pct', np.nan))}\n"
        f"RMS right / RMS left = {fmt_pct(metrics.get('rmsdiff_over_rms_reference_pct', np.nan))}\n"
        f"RMS right / RMS center = {fmt_pct(metrics.get('rmsdiff_over_rms_comparison_pct', np.nan))}"
    )
    axes[2].text(
        0.02,
        0.98,

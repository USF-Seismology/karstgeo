...
SHOT_DIR="${SHOT_DIR:-SURVEY_OUTPUT/shot_001_xs00134p5}"
...
run_one_frequency_component() {
  ...
  ref_file="$ref_root/$SHOT_DIR/$component_file"
  cmp_file="$cmp_root/$SHOT_DIR/$component_file"
  ...
  run_bg "$log" "$PYTHON" "$ENGINE" \
    ...
    --reference-pattern "$SHOT_DIR/$component_file" \
    --comparison-pattern "$SHOT_DIR/$component_file" \

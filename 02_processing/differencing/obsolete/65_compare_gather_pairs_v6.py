...
def best_lag_ms(a, b, dt_s, max_lag_s=None) -> tuple[float, float]:
    """
    Return best lag, in ms, and correlation at that lag.

    Convention:
      positive lag means b is delayed relative to a by that lag.
    """
    a = np.asarray(a, dtype=float).ravel()
    b = np.asarray(b, dtype=float).ravel()
    n = min(a.size, b.size)
    ...
    den = np.sqrt(np.sum(a * a) * np.sum(b * b))
    corr = float(cc[i] / den) if den else np.nan

    # np.correlate(a,b): positive lag here means a must be shifted right relative to b.
    # For user convention "b delayed relative to a", flip sign.
    return float(-lag_samples * dt_s * 1000.0), corr
...
def compute_trace_similarity_metrics(
    ...
    *,
    ...
    for i, x in enumerate(rx):
        ...
        lag_ms, lag_corr = best_lag_ms(ref, cmp, dt, max_lag_s=max_lag_s)
        rows.append({
            "metric_set": label,
...
def apply_fk_filter_to_matrix(
    ...
    *,
    ...
    Preferred implementation uses segy_tools.spectral.apply_fk_velocity_filter
    when available. A compact fallback implementation is included so this
    script remains runnable if segy_tools is not importable.
    """
    dt = float(np.nanmedian(np.diff(t)))
    dx = float(np.nanmedian(np.diff(np.sort(rx)))) if len(rx) > 1 else 1.0
    st = matrix_to_stream(data, dt, receiver_x_m=rx, source_x_m=source_x_m)

    if segy_spectral is not None and hasattr(segy_spectral, "apply_fk_velocity_filter"):
        filtered = segy_spectral.apply_fk_velocity_filter(
            st,
            min_velocity_mps=float(min_velocity_mps),
            receiver_spacing_m=dx,
            use_taper=bool(use_taper),
            taper_width_mps=float(taper_width_mps),
        )
        return stream_to_matrix_exact(filtered, n_samples=data.shape[1])

    # Fallback based on the same f-k fan-filter concept.
    ...
    if use_taper:
        ...
        x = (apparent_velocity[transition] - v0) / max(v1 - v0, 1e-12)
        mask[transition] = 0.5 * (1.0 - np.cos(np.pi * x))
    else:
        mask[apparent_velocity < v0] = 0.0

    # Preserve k=0 content, matching segy_tools.spectral behavior.
    mask[k_grid == 0] = 1.0
    return np.real(np.fft.ifft2(fk * mask))
...
def write_diagnostic_comparison_product(
    *,
    ...
    plot_three_panel(
        ...
        outfile=outdir / "comparison_image_reference_comparison_difference.png",
    ...
    if args.write_trace_normalized_figures:
        ...
        plot_three_panel(
            ...
            outfile=outdir / "comparison_image_reference_comparison_difference_trace_normalized.png",
            cave_extent=cave_extent,
...
def annotate_cave_and_source(ax, source_x_m: float, cave_extent: Optional[tuple[float, float]], color_source="k", color_cave="0.5"):
    if cave_extent is not None:
        ax.axvspan(cave_extent[0], cave_extent[1], color=color_cave, alpha=0.15)
...
def plot_three_panel(
    ...
    *,
    ...
    diff_title = (
        f"Difference: {reference_label} - {comparison_label}\n"
        f"peak diff = {fmt_pct(peak_ref_pct)} of left, {fmt_pct(peak_cmp_pct)} of center; "
        f"RMS diff = {fmt_pct(rms_ref_pct)} of left, {fmt_pct(rms_cmp_pct)} of center"
    )
    ...
    for ax, (data, title, c) in zip(axes, panels):
        ...
        annotate_cave_and_source(ax, source_x_m, cave_extent)
        ax.set_title(title)
    ...
    fig.text(
        ...
        0.005,
        (
            f"Difference amplitude: peak right/left={fmt_pct(peak_ref_pct)}, "
            f"peak right/center={fmt_pct(peak_cmp_pct)}, "
            f"RMS right/left={fmt_pct(rms_ref_pct)}, "
            f"RMS right/center={fmt_pct(rms_cmp_pct)}"
        ),
        ha="center",
        va="bottom",
        fontsize=9,
...
def plot_frequency(
    ...
    *,
    ...
    if cave_extent is not None:
        ax.axvspan(cave_extent[0], cave_extent[1], color="w", alpha=0.18)
...
def plot_trace_similarity_summary(
    ...
    *,
    ...
    """Plot per-trace similarity metrics versus receiver position."""
    if not rows:
        return
    outfile.parent.mkdir(parents=True, exist_ok=True)
    ...
    lag_ms = np.asarray([r["best_lag_ms"] for r in rows], dtype=float)
    lagcorr = np.asarray([r["best_lag_corr"] for r in rows], dtype=float)

    fig, axes = plt.subplots(4, 1, figsize=(11, 9), sharex=True)

    panels = [
        (zcorr, "zero-lag correlation", (-1.05, 1.05)),
        (lagcorr, "best-lag correlation", (-1.05, 1.05)),
        (nrmse_pct, "NRMSE vs reference RMS (%)", None),
        (lag_ms, "best lag (ms)", None),
    ]
    ...
    mean_corr = np.nanmean(zcorr)
    med_corr = np.nanmedian(zcorr)
    mean_nrmse = np.nanmean(nrmse_pct)

    axes[0].set_title(
        f"{title}\n"
        f"mean zero-lag r={mean_corr:.4f}, median r={med_corr:.4f}; "
        f"mean NRMSE={mean_nrmse:.2f}%"
    )
...
def plot_spectral_contours_three_panel(
    ...
    *,
    ...
    """
    Combined Charlie-style spectral contour figure.

    Important: the color scale is shared across reference, comparison and
    difference so the difference panel cannot appear artificially large simply
    because it received its own independent color scale.
    """
    outfile.parent.mkdir(parents=True, exist_ok=True)

    freqs, arrays = _shared_spectral_arrays(
        t,
        [reference, comparison, diff],
    ...
    titles = [
        ...
        f"Difference: reference - comparison",
    ]

    cf = None
    for ax, z, title in zip(axes, arrays, titles):
        cf = ax.contourf(
            X, Y, z.T,
            levels=np.linspace(vmin, vmax, int(levels)),
            cmap="viridis",
    ...
    if cf is not None:
        cb = fig.colorbar(cf, ax=axes, fraction=0.028, pad=0.02)
        cb.set_label(label)
...
def plot_frequency_three_panel(
    ...
    *,
    ...
    im = None
    for ax, amp, title in zip(axes, arrays, titles):
        im = ax.imshow(
            amp.T,
            aspect="auto",
            origin="lower",
...
def plot_band_energy_three_panel(
    ...
    *,
    ...
    """
    Combined band-energy figure.

    Layout:
      rows = frequency bands
      columns = reference, comparison, difference

    Color scale is shared per frequency band across the three columns.
    """
    outfile.parent.mkdir(parents=True, exist_ok=True)
    ...
    fig, axes = plt.subplots(nrows, 3, figsize=(18, max(4.0 * nrows, 5.5)), sharex=True, sharey=True)
    if nrows == 1:
    ...
    for r, band_label in enumerate(band_labels):
        ...
        if log10:
            finite = np.concatenate([a[np.isfinite(a)].ravel() for a in band_arrays])
            vmin = float(np.nanpercentile(finite, 2)) if finite.size else -1
            vmax = float(np.nanpercentile(finite, 98)) if finite.size else 1
        else:
            vmin = 0.0
            vmax = clip_value(*band_arrays, percentile=98.0)
        ...
        if im is not None:
            cb = fig.colorbar(im, ax=axes[r, :], fraction=0.018, pad=0.01)
            cb.set_label("log10 band energy" if log10 else ("trace-normalized band energy" if normalize_per_trace else "band energy"))

    diff_metrics = comparison_metrics(reference, comparison, diff)
    fig.suptitle(
        f"Band energy, source x={source_x_m:.3f} m; shared scale within each band; "
        f"RMS diff/ref={fmt_pct(diff_metrics['rmsdiff_over_rms_reference_pct'])}",
        y=0.995,
    )
...
def process_pair(pair: Pair, *, stations: list[Station], args, preprocess_kwargs: dict, cave_extent, bands) -> tuple[PairResult, list[dict]]:
    ...
    plot_three_panel(
        ...
        reference_label=pair.reference_label,
        comparison_label=pair.comparison_label,
        outfile=shot_dir / "comparison_image_reference_comparison_difference.png",
        cave_extent=cave_extent,
        metrics=metrics,
        title_extra=f"; scale={args.scale_mode}, factor={sfactor:.5g}; physical/scaled amplitudes",
    )
    ...
    if args.write_trace_normalized_figures:
        ...
        plot_three_panel(
            ...
            outfile=shot_dir / "comparison_image_reference_comparison_difference_trace_normalized.png",
            cave_extent=cave_extent,
            metrics=norm_metrics,
            title_extra=(
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--fk-min-velocity-mps", type=float, default=1000.0,
                   help="Reject apparent velocities below this value in optional f-k filter. Default: 1000 m/s.")
    p.add_argument("--fk-taper-width-mps", type=float, default=200.0,
                   help="Raised-cosine transition width for optional f-k filter. Default: 200 m/s.")
    ...
    p.add_argument("--frequency-trace-normalization", action=argparse.BooleanOptionalAction, default=True)
    ...
    p.add_argument("--similarity-max-lag-s", type=float, default=0.02,
                   help="Maximum lag for best-lag correlation. Default: 0.02 s.")

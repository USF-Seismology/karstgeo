...
def plot_three_panel(
    ...
    *,
    ...
    panels = [
        ...
        (diff, f"Difference: {reference_label} - {comparison_label}", diff_c),
    ]
    ...
    txt = (
        f"max|diff| / max|reference| = {fmt_pct(metrics.get('maxdiff_over_max_reference_pct', np.nan))}\n"
        f"max|diff| / max|comparison| = {fmt_pct(metrics.get('maxdiff_over_max_comparison_pct', np.nan))}\n"
        f"rms(diff) / rms(reference) = {fmt_pct(metrics.get('rmsdiff_over_rms_reference_pct', np.nan))}\n"
        f"rms(diff) / rms(comparison) = {fmt_pct(metrics.get('rmsdiff_over_rms_comparison_pct', np.nan))}"
    )
    axes[2].text(
        0.02,
        0.98,
...
def plot_overlay_wiggles(
    ...
    *,
    ...
    ax.set_title(f"Overlay wiggles: comparison blue, reference red; source x={source_x_m:.3f} m")
    ax.set_xlabel("receiver x (m)")
    ax.set_ylabel("time (s)")
    ax.grid(alpha=0.15)
    ax.invert_yaxis()

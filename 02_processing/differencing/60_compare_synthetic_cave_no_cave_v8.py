...
def plot_wiggle(
    *,
    ...
    if cave_x_m is not None:
        ax.axvspan(cave_x_m[0], cave_x_m[1], color="0.5", alpha=0.15, label="cave/void")
...
def plot_frequency_offset(
    *,
    ...
    if cave_x_m is not None:
        ax.axvspan(cave_x_m[0], cave_x_m[1], color="w", alpha=0.18, label="cave/void")
...
def process_pair(
    *,
    ...
    write_band_energy: bool,
    band_energy_bands: list[tuple[float, float]],
    band_energy_window_s: float,
    band_energy_step_s: float,
    band_energy_normalize_per_trace: bool,
    band_energy_log10: bool,
    ...
    if write_band_energy:
        plot_band_energy(
            data=cave,
            time_s=time_s,
            receiver_x_m=receiver_x_m,
            source_x_m=source_x_m,
            title=f"Band energy: WITH cave, source x = {source_x_m:.3f} m",
            outfile=shot_dir / "band_energy_with_cave.png",
            bands=band_energy_bands,
            window_s=band_energy_window_s,
            step_s=band_energy_step_s,
            normalize_per_trace=band_energy_normalize_per_trace,
            log10=band_energy_log10,
            cave_x_m=cave_extent_x_m,
        )
        plot_band_energy(
            data=nocave,
            ...
            receiver_x_m=receiver_x_m,
            source_x_m=source_x_m,
            title=f"Band energy: WITHOUT cave, source x = {source_x_m:.3f} m",
            outfile=shot_dir / "band_energy_without_cave.png",
            bands=band_energy_bands,
            window_s=band_energy_window_s,
            step_s=band_energy_step_s,
            normalize_per_trace=band_energy_normalize_per_trace,
        ...
        plot_band_energy(
            ...
            receiver_x_m=receiver_x_m,
            source_x_m=source_x_m,
            title=f"Band energy: Difference cave - no cave, source x = {source_x_m:.3f} m",
            outfile=shot_dir / "band_energy_difference_cave_minus_nocave.png",
            bands=band_energy_bands,
            window_s=band_energy_window_s,
            step_s=band_energy_step_s,
            normalize_per_trace=band_energy_normalize_per_trace,
        ...
        if write_trace_normalized_figures:
            plot_band_energy(
                ...
                receiver_x_m=receiver_x_m,
                source_x_m=source_x_m,
                title=f"Band energy: Trace-normalized difference cave - no cave, source x = {source_x_m:.3f} m",
...
def _par_float(value: str) -> float:
...
def parse_void_extent_from_par_file(
    ...
    *,
    ...
    """
    Parse approximate horizontal void/cave extent from a SPECFEM2D Par_file.

    Assumptions for regular internal mesh:
      - xmin, xmax, nx define x coordinate and element count
      - nbmodels block contains material definitions
      - void material can be supplied explicitly, or inferred from Vs == 0
      - nbregions block contains ixmin ixmax izmin izmax material_id

    Conversion from element indices to x:
        ...
        for line in lines[nbmodels_idx + 1:]:
            ...
            if len(parts) >= 5 and parts[0].lstrip("+-").isdigit():
                try:
                    ...
                    if abs(vs) < 1e-12:
                        model_ids_with_zero_vs.add(mid)
                    count += 1
                    if count >= nbmodels:
                        break
                except Exception:
    ...
    target_ids = {int(void_material_id)} if void_material_id is not None else model_ids_with_zero_vs
    if not target_ids:
        return None
...
def resolve_cave_extent(args) -> Optional[tuple[float, float]]:
    ...
    if par_file is not None and par_file.exists():
        extent = parse_void_extent_from_par_file(
            par_file,
            void_material_id=args.void_material_id,
        )
    ...
    if args.cave_extent_x_m:
        extent = parse_cave_extent(args.cave_extent_x_m)
        print(
    ...
    for i, (source_label, source_x_m, cave_path, nocave_path) in enumerate(pairs, start=1):
        ...
        try:
            res, peak_rows = process_pair(
                ...
                write_band_energy=args.write_band_energy,
                band_energy_bands=band_energy_bands,
                band_energy_window_s=args.band_energy_window_s,
                band_energy_step_s=args.band_energy_step_s,
                band_energy_normalize_per_trace=args.band_energy_normalize_per_trace,
                band_energy_log10=args.band_energy_log10,
            )
            results.append(res)

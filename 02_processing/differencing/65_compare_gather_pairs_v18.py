...
def shift_x_extent(extent, shift_m: float):
    """Shift a cave/void x-extent object without changing panel metadata.

    extent is usually a simple (xmin, xmax) tuple. In newer versions it may also
    be a dict like {"extent": (xmin, xmax), "panels": {...}} so that cave
    shading appears only in selected three-panel products.
    """
    if extent is None:
        return None
    if not shift_m:
    ...
    if isinstance(extent, dict):
        shifted = dict(extent)
        inner = extent.get("extent")
        shifted["extent"] = shift_x_extent(inner, shift_m)
        return shifted
    xmin, xmax = extent
    return (float(xmin) + float(shift_m), float(xmax) + float(shift_m))
...
def robust_rms(values: np.ndarray, *, trim_percentile: float = 100.0) -> float:
    """RMS with optional amplitude-percentile trimming to reduce spike leverage."""
    x = np.asarray(values, dtype=float).ravel()
    x = x[np.isfinite(x)]
    if x.size == 0:
        return 0.0
    if trim_percentile is not None and np.isfinite(trim_percentile) and trim_percentile < 100.0:
        trim_percentile = max(0.0, min(100.0, float(trim_percentile)))
        limit = np.nanpercentile(np.abs(x), trim_percentile)
        if np.isfinite(limit) and limit > 0:
            x = x[np.abs(x) <= limit]
    return rms(x)


def _scale_window_mask(
    t: np.ndarray,
    *,
    scale_window_s: Optional[tuple[float, float]],
    scale_tmin: Optional[float],
    scale_tmax: Optional[float],
) -> np.ndarray:
    keep = np.ones_like(t, dtype=bool)
    if scale_window_s is not None:
        scale_tmin, scale_tmax = scale_window_s
...
def compute_comparison_scaling(
    reference: np.ndarray,
    comparison: np.ndarray,
    t: np.ndarray,
    *,
    ...
    """
    Return scaled comparison, representative scale factor, and per-trace scale rows.

    Modes:
      none/fixed/rms/lsq/maxabs/global_rms/global_peak -> one scale for whole gather
      trace_rms/trace_peak -> independent scale per trace
    """
    ...
    keep = _scale_window_mask(
        t,
        scale_window_s=scale_window_s,
        scale_tmin=scale_tmin,
        scale_tmax=scale_tmax,
    )
    r = reference[:, keep]
    c = comparison[:, keep]

    def clip_gain(g: float) -> float:
        if not np.isfinite(g) or g == 0:
            return 1.0
        lo = float(clip_min) if clip_min is not None else -np.inf
        hi = float(clip_max) if clip_max is not None else np.inf
        if np.isfinite(lo):
            g = max(lo, g)
        if np.isfinite(hi):
            g = min(hi, g)
    ...
    if mode == "none":
        return comparison.copy(), 1.0, rows
    if mode == "fixed":
        sf = float(fixed_scale)
        return comparison * sf, sf, rows
    if mode == "rms":
        den = robust_rms(c, trim_percentile=trim_percentile)
        sf = robust_rms(r, trim_percentile=trim_percentile) / den if den else 1.0
        sf = clip_gain(sf)
        return comparison * sf, sf, rows
    if mode == "lsq":
        sf = clip_gain(lsq_scale(r, c))
        return comparison * sf, sf, rows
    if mode == "maxabs":
        den = float(np.nanmax(np.abs(c))) if c.size else 0.0
        sf = float(np.nanmax(np.abs(r)) / den) if den else 1.0
        sf = clip_gain(sf)
        return comparison * sf, sf, rows

    if mode not in {"trace_rms", "trace_peak"}:
        raise ValueError(f"Unknown scale mode: {mode}")

    gains = np.ones(reference.shape[0], dtype=float)
    for i in range(reference.shape[0]):
        ri = r[i, :]
        ci = c[i, :]
        if mode == "trace_rms":
            den = robust_rms(ci, trim_percentile=trim_percentile)
            raw_gain = robust_rms(ri, trim_percentile=trim_percentile) / den if den else 1.0
        else:  # trace_peak
            den = float(np.nanmax(np.abs(ci))) if ci.size else 0.0
            raw_gain = float(np.nanmax(np.abs(ri)) / den) if den else 1.0
        gain = clip_gain(raw_gain)
        gains[i] = gain
        rows.append({
            "trace_index": i,
            "raw_gain_reference_over_comparison": raw_gain,
            "clipped_gain_reference_over_comparison": gain,
            "scale_mode": mode,
            "scale_window_tmin_s": float(t[keep][0]) if np.any(keep) else np.nan,
            "scale_window_tmax_s": float(t[keep][-1]) if np.any(keep) else np.nan,
            "scale_trim_percentile": trim_percentile,
            "scale_clip_min": clip_min,
            "scale_clip_max": clip_max,
            "reference_robust_rms": robust_rms(ri, trim_percentile=trim_percentile),
            "comparison_robust_rms": robust_rms(ci, trim_percentile=trim_percentile),
            "reference_peak_abs": float(np.nanmax(np.abs(ri))) if ri.size else np.nan,
            "comparison_peak_abs": float(np.nanmax(np.abs(ci))) if ci.size else np.nan,
        })

    scaled = comparison * gains[:, None]
    representative = float(np.nanmedian(gains)) if gains.size else 1.0
    return scaled, representative, rows
...
def apply_fk_filter_to_matrix(
    ...
    *,
    ...
    Optional split-at-source filtering is available, but the default is now the
    Sarah-compatible global f-k filter:
...
def write_diagnostic_comparison_product(
    *,
    ...
    # NOTE: Removed stale duplicate diagnostic combined-wiggle block that referenced
    # outer process_pair variables (ref/cmp_scaled) and wrote to shot_dir.
    ...
    # NOTE: Removed stale duplicate diagnostic spectral-contour block that referenced
    # outer process_pair variables (ref/cmp_scaled) and wrote to shot_dir.
...
def _segy_safe_int(value: float, scale: float = 1000.0, default: int = 0) -> int:
    """Convert metres to a SEG-Y integer coordinate using a positive multiplier."""
    try:
        v = float(value)
    except Exception:
        return int(default)
...
def _slug_token(text: str, max_len: int = 80) -> str:
    """Filesystem-safe token for labels and gather-type names."""
    ...
    return (token[:max_len].strip("_") or "gather")


def infer_gather_type(label: str, path: Path | str | None = None, *, role: str = "") -> str:
    """Infer a concise gather type for output filenames from labels/paths."""
    hay = f"{label} {path or ''} {role}".lower()
    is_real = "real" in hay or "field" in hay or ".dat" in hay or "seg2" in hay
    is_no_void = any(x in hay for x in ["no_void", "no void", "novoid", "no-void", "without cave", "without_a_cave"])
    is_cave = any(x in hay for x in ["polygon", "void", "cave", "rectangle", "with cave", "with_cave"])
    is_synth = any(x in hay for x in ["synthetic", "specfem", "sources_grounded", "simulation", ".su", ".sgy", ".segy"])
    if is_real and is_synth:
        # Prefer real only when the label/path clearly describes a real field file.
        if ".dat" in hay or "field" in hay:
            return "real"
    if is_real:
        return "real"
    if is_no_void:
        return "synthetic_without_cave"
    if is_cave:
        return "synthetic_with_cave"
    if is_synth:
        return "synthetic"
    return _slug_token(label or role)


def difference_gather_type(reference_type: str, comparison_type: str) -> str:
    """Return a readable type for reference-minus-comparison gathers."""
    if reference_type == "real" and comparison_type == "synthetic_without_cave":
        return "real_minus_synthetic_without_cave"
    if reference_type == "real" and comparison_type == "synthetic_with_cave":
        return "real_minus_synthetic_with_cave"
    if reference_type == "synthetic_with_cave" and comparison_type == "synthetic_without_cave":
        return "synthetic_with_cave_minus_synthetic_without_cave"
    if reference_type == "synthetic_without_cave" and comparison_type == "synthetic_with_cave":
        return "synthetic_without_cave_minus_synthetic_with_cave"
    return f"{_slug_token(reference_type)}_minus_{_slug_token(comparison_type)}"


def shot_x_token(source_x_m: float) -> str:
    """Stable shot-position token such as shot_x0134p5m or shot_xm0002p5m."""
    x = float(source_x_m)
    sign = "m" if x < 0 else ""
    return f"shot_x{sign}{abs(x):07.1f}m".replace(".", "p")


def build_gather_segy_filename(source_x_m: float, gather_type: str, processing: str, role: str = "") -> str:
    """Filename containing shot position, gather type, processing, and role."""
    parts = [shot_x_token(source_x_m), _slug_token(gather_type), _slug_token(processing)]
    role_token = _slug_token(role)
    if role_token and role_token not in {"gather", "none"}:
        parts.append(role_token)
    return "__".join(parts) + ".sgy"


def write_gather_segy(
    outfile: Path,
    data: np.ndarray,
    dt_s: float,
    rx: np.ndarray,
    source_x_m: float,
    *,
    gather_type: str = "gather",
    role: str = "",
    processing: str = "processed",
    source_id: str | None = None,
    label: str = "",
) -> None:
    """Write one processed gather matrix to SEG-Y with metre geometry encoded.

    Coordinates are written as integer millimetres with SEG-Y coordinate scalar
    -1000, so downstream software should recover metres. The same geometry is
    also retained in ObsPy stats fields for tools that read those custom values.
    """
    outfile.parent.mkdir(parents=True, exist_ok=True)
    data = np.asarray(data, dtype=np.float32)
    rx = np.asarray(rx, dtype=float)
    if data.ndim != 2:
        raise ValueError(f"Expected 2-D gather data for SEG-Y writing, got {data.shape}")
    if data.shape[0] != rx.size:
        raise ValueError(f"SEG-Y geometry mismatch: data has {data.shape[0]} traces but rx has {rx.size}")

    sx_i = _segy_safe_int(source_x_m, 1000.0)
    shot_record = int(round(float(source_x_m) * 10.0)) if np.isfinite(source_x_m) else 0

    st = Stream()
    for i, (x, trdata) in enumerate(zip(rx, data), start=1):
        gx_i = _segy_safe_int(float(x), 1000.0)
        tr = Trace(data=np.asarray(trdata, dtype=np.float32))
        tr.stats.delta = float(dt_s)
        tr.stats.network = "KG"
        tr.stats.station = f"R{i:04d}"
        tr.stats.channel = "Z"
        tr.stats.receiver_x_m = float(x)
        tr.stats.source_x_m = float(source_x_m)
        tr.stats.gather_type = gather_type
        tr.stats.processing = processing
        if source_id is not None:
            tr.stats.source_id = str(source_id)

        tr.stats.segy = {}
        tr.stats.segy.trace_header = SEGYTraceHeader()
        hdr = tr.stats.segy.trace_header
        hdr.trace_sequence_number_within_line = i
        hdr.trace_number_within_the_original_field_record = i
        hdr.original_field_record_number = shot_record
        hdr.energy_source_point_number = shot_record
        hdr.scalar_to_be_applied_to_all_coordinates = -1000
        hdr.source_coordinate_x = sx_i
        hdr.source_coordinate_y = 0
        hdr.group_coordinate_x = gx_i
        hdr.group_coordinate_y = 0
        hdr.distance_from_center_of_the_source_point_to_the_center_of_the_receiver_group = int(round((float(x) - float(source_x_m)) * 1000.0))
        # Commonly used trace header slots for quick identification.
        hdr.trace_identification_code = 1
        hdr.number_of_samples_in_this_trace = int(data.shape[1])
        hdr.sample_interval_in_ms_for_this_trace = int(round(float(dt_s) * 1_000_000.0))
        st.append(tr)

    st.write(str(outfile), format="SEGY", data_encoding=1)


def write_pair_gather_segys(
    outdir: Path,
    *,
    reference: np.ndarray,
    comparison: np.ndarray,
    ...
    """Write reference, comparison, and reference-minus-comparison SEG-Y gathers."""
    ref_type = infer_gather_type(pair.reference_label, pair.reference_path, role="reference")
    cmp_type = infer_gather_type(pair.comparison_label, pair.comparison_path, role="comparison")
    diff_type = difference_gather_type(ref_type, cmp_type)
    entries = [
        ("reference", ref_type, reference, pair.reference_label),
        ("comparison", cmp_type, comparison, pair.comparison_label),
        ("difference_reference_minus_comparison", diff_type, difference, f"{pair.reference_label} minus {pair.comparison_label}"),
    ]
    segy_dir = outdir / "segy" / _slug_token(processing)
    for role, gtype, matrix, label in entries:
        fname = build_gather_segy_filename(pair.source.x_m, gtype, processing, role)
        write_gather_segy(
            segy_dir / fname,
            matrix,
            dt_s,
...
def write_diff_segy(
    ...
    """Backward-compatible difference-only SEG-Y writer."""
    write_gather_segy(
        outfile,
        diff,
        dt_s,
        rx,
        source_x_m,
        gather_type="difference_reference_minus_comparison",
        role="difference",
...
def process_pair(pair: Pair, *, stations: list[Station], args, preprocess_kwargs: dict, cave_extent, bands) -> tuple[PairResult, list[dict]]:
    ...
    # Optional display-coordinate shift. This is deliberately applied only after
    # real/synthetic shot and receiver matching, so it does not affect which
    # traces are compared. It only changes x-axis labels, source marker position,
    # cave shading, output metrics x values, and optional output SEG-Y geometry.
    display_x_shift_m = float(getattr(args, "display_x_shift_m", 0.0) or 0.0)
    if display_x_shift_m:
        rx = rx + display_x_shift_m
        cave_extent = shift_x_extent(cave_extent, display_x_shift_m)
        pair = Pair(
            shot_index=pair.shot_index,
            source=Source(
                source_id=pair.source.source_id,
                x_m=pair.source.x_m + display_x_shift_m,
                z_m=pair.source.z_m,
                raw=pair.source.raw,
            ),
    ...
    cmp_scaled, sfactor, trace_scale_rows = compute_comparison_scaling(
        ref,
        cmp_raw,
        t,
        ...
        scale_window_s=args.scale_window_s,
        trim_percentile=args.scale_trim_percentile,
        clip_min=args.scale_clip_min,
        clip_max=args.scale_clip_max,
    )
    ...
    if trace_scale_rows:
        for row, receiver_x in zip(trace_scale_rows, rx):
            row.update({
        ...
        write_csv(shot_dir / "trace_amplitude_scaling_factors.csv", trace_scale_rows)
    ...
    plot_three_panel(
        ...
        metrics=metrics,
        title_extra=(
            f"; scale={args.scale_mode}, representative factor={sfactor:.5g}; "
            f"synthetic source target={args.synthetic_source_target_factor:.5g}; "
            "physical/scaled amplitudes"
        ),
    )

    if args.write_gather_segys:
        write_pair_gather_segys(
            shot_dir,
            reference=ref,
            ...
            rx=rx,
            pair=pair,
            processing=args.gather_segy_processed_dirname,
        )
    ...
    if args.write_fk_filtered:
        ...
        if args.write_gather_segys or args.write_fk_gather_segys:
            fk_diff = fk_ref - fk_cmp
            write_pair_gather_segys(
                shot_dir,
                reference=fk_ref,
                comparison=fk_cmp,
                ...
                rx=rx,
                pair=pair,
                processing=f"fk_vmin_{args.fk_min_velocity_mps:g}mps",
            )
...
def parse_time_window(text: Optional[str]) -> Optional[tuple[float, float]]:
    if not text:
        return None
    parts = [p.strip() for p in text.split(",")]
    if len(parts) != 2:
        raise argparse.ArgumentTypeError("time window must be like 0.02,0.20")
    t0, t1 = float(parts[0]), float(parts[1])
    if not np.isfinite(t0) or not np.isfinite(t1) or t1 <= t0:
        raise argparse.ArgumentTypeError("time window must have finite tmin,tmax with tmax > tmin")
    return t0, t1
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        ...
        default=0.0,
        help=(
            "Shift x coordinates after matching, for display/output only. "
            "Use this when real data were matched to a synthetic model in an "
            "offset coordinate frame but figures/CSVs should be reported in "
            "local survey coordinates. Example: T3/T4 matched to T1 model with "
            "real-first-trace-x-m=87 can be plotted as 0..71 using "
            "--display-x-shift-m -87."
        ),
    )

    p.add_argument("--scale-mode", choices=["none", "fixed", "rms", "lsq", "maxabs", "global_rms", "global_peak", "trace_rms", "trace_peak"], default="none")
    ...
    p.add_argument("--scale-window-s", type=parse_time_window, default=None,
                   help="Preferred scale window as tmin,tmax in seconds, e.g. 0.02,0.20. Overrides --scale-tmin/--scale-tmax when provided.")
    p.add_argument("--scale-trim-percentile", type=float, default=100.0,
                   help="Amplitude percentile retained for robust RMS scaling. Use 99 to reduce spike leverage. Default: 100, no trimming.")
    p.add_argument("--scale-clip-min", type=float, default=0.0,
                   help="Minimum allowed amplitude scale factor. Default: 0, effectively no lower clipping.")
    p.add_argument("--scale-clip-max", type=float, default=np.inf,
                   help="Maximum allowed amplitude scale factor. Default: inf, no upper clipping.")
    ...
    p.add_argument("--fk-split-at-source", action=argparse.BooleanOptionalAction, default=False,
                   help="Apply f-k filter separately to receivers left/right of source. Default: false. The global filter matches Sarah/core_energy.py more closely and avoids source-split seam artifacts.")
    p.add_argument("--fk-spatial-taper-fraction", type=float, default=0.0,
                   help="Fraction of receiver aperture tapered before f-k FFT. Default: 0.0 to match Sarah/core_energy.py.")
    p.add_argument("--fk-pad-factor", type=int, default=1,
                   help="Zero-padding factor along receiver axis before f-k FFT. Default: 1 to match Sarah/core_energy.py.")
    ...
    p.add_argument("--write-gather-segys", action=argparse.BooleanOptionalAction, default=False,
                   help="Write SEG-Y versions of the processed reference, comparison, and reference-minus-comparison gathers used in the main plots.")
    p.add_argument("--write-fk-gather-segys", action=argparse.BooleanOptionalAction, default=False,
                   help="Write SEG-Y versions of f-k filtered reference, comparison, and difference gathers. Implies useful output when --write-fk-filtered is also enabled.")
    p.add_argument("--gather-segy-processed-dirname", default="processed",
                   help="Subfolder name under shot_dir/segy for main processed-but-not-f-k-filtered gather SEG-Y files.")

...
Default scale mode is none. This is deliberate because real/synthetic amplitude
calibration is not yet settled. Try --scale-mode rms, maxabs, or lsq later.
"""
...
def read_real_gather(
    ...
    *,
    ...
    st = detrend_demean(read_stream_any(path), demean=True, detrend=True)
...
def read_synthetic_gather(
    ...
    *,
    ...
    st = detrend_demean(read_stream_any(path), demean=True, detrend=False)
    data, dt, time_s = stream_to_matrix(st)
...
def align(real: Gather, syn: Gather, *, receiver_tol_m: float, tmin: float, tmax: float):
    pairs = []
    used = set()
    ...
    # Use real time axis as authoritative, and interpolate synthetic onto it.
    t = real.time_s.copy()
...
def scale_factor(real: np.ndarray, syn: np.ndarray, t: np.ndarray, mode: str, scale_tmin, scale_tmax) -> float:
    mode = mode.lower()
    if mode == "none":
        return 1.0
...
def build_parser():
    ...
    p.add_argument("--scale-mode", choices=["none", "maxabs", "rms", "lsq"], default="none")
...
def main(argv=None):
    ...
    fields = [
        ...
        "scale_mode", "scale_factor", "n_receivers", "n_samples", "dt_s",

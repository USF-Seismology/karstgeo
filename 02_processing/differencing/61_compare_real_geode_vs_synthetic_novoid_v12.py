...
def plot_wiggle(t, rx, data, source_x, title, out, cave_extent=None):
    out.parent.mkdir(parents=True, exist_ok=True)
...
def plot_freq(t, rx, data, source_x, title, out, max_freq, normalize=True, cave_extent=None):
    out.parent.mkdir(parents=True, exist_ok=True)
...
def plot_overlay_wiggles(t, rx, real, syn, source_x, out, *, normalize="pair", wiggle_scale=0.45, title_extra="", cave_extent=None):
    """
...
def sliding_band_energy(
    ...
    *,
    ...
    """
    Compute short-time band energy images.

    Returns:
        centers_s: time centers of each window
        band_images: dict label -> array shape (n_receivers, n_windows)

    Energy is computed from windowed FFT amplitude in each time window.
    This is an S-transform/spectrogram-like diagnostic, but intentionally
    simpler and easier to interpret.
    """
    d = np.asarray(data, dtype=float)
    t = np.asarray(time_s, dtype=float)
    if d.ndim != 2:
        raise ValueError("data must be 2-D: n_traces x n_samples")
    if t.size != d.shape[1]:
        raise ValueError("time_s length must match data sample count")
    if t.size < 4:
        raise ValueError("Too few time samples for band energy")

    dt = float(np.nanmedian(np.diff(t)))
    if not np.isfinite(dt) or dt <= 0:
        raise ValueError("Invalid time sampling")
...
def plot_band_energy(
    *,
    ...
    """
    Plot sliding-window band energy as one panel per frequency band.
    """
    outfile.parent.mkdir(parents=True, exist_ok=True)

    centers, band_images = sliding_band_energy(
        data,
        time_s,
        bands,
        window_s=window_s,
    ...
    n = len(band_images)
    fig_w = max(5.0 * n, 8.0)
    fig, axes = plt.subplots(1, n, figsize=(fig_w, 6), sharey=True)
    if n == 1:
        axes = [axes]

    extent = [
        float(np.nanmin(receiver_x_m)),
        float(np.nanmax(receiver_x_m)),
        float(np.nanmax(centers)),
        float(np.nanmin(centers)),
    ]

    for ax, (label, img) in zip(axes, band_images.items()):
        clim = _clip_value(img, percentile=clip_percentile)
        ...
        if cave_x_m is not None:
            ax.axvspan(cave_x_m[0], cave_x_m[1], color="w", alpha=0.18)
    ...
    fig.suptitle(f"{title}; sliding-window band energy ({mode}), window={window_s:g}s, step={step_s:g}s", y=0.99)
    fig.tight_layout()
...
def _par_float(value):
    return float(str(value).strip().replace("d", "e").replace("D", "E"))


def parse_void_extent_from_par_file(par_file, void_material_id=None):
    ...
    for line in lines:
        ...
        if key.lower() == "xmin":
            xmin = _par_float(value)
        elif key.lower() == "xmax":
            xmax = _par_float(value)
        elif key.lower() == "nx":
    ...
    target_ids = {int(void_material_id)} if void_material_id is not None else zero_vs_ids
    if not target_ids:
        return None
...
def resolve_cave_extent(args):
    par_file = Path(args.par_file) if getattr(args, "par_file", None) else find_par_file(
        args.synthetic_novoid_dir,
        args.data_dir,
    )
    if par_file is not None and par_file.exists():
        extent = parse_void_extent_from_par_file(par_file, args.void_material_id)
...
def build_parser():
    ...
    p.add_argument("--cave-extent-x-m", default=None,
                   help="Fallback/manual cave x extent to shade, e.g. '140,160'.")
    p.add_argument("--par-file", type=Path, default=None,
                   help="Optional SPECFEM2D Par_file from which to parse void/cave x extent.")
    p.add_argument("--void-material-id", type=int, default=None,
                   help="Optional material id for void/cave. If omitted, infer material with Vs=0 from Par_file.")
    ...
    p.add_argument("--write-band-energy", action=argparse.BooleanOptionalAction, default=True,
                   help="Write sliding-window band-energy images. Default: true.")
    p.add_argument("--band-energy-bands", default="10-30,30-80,80-150",
                   help="Frequency bands for band-energy plots, e.g. '10-30,30-80,80-150'.")
    p.add_argument("--band-energy-window-s", type=float, default=0.05,
                   help="Sliding time window length for band-energy plots. Default: 0.05 s.")
    p.add_argument("--band-energy-step-s", type=float, default=0.01,
                   help="Sliding time window step for band-energy plots. Default: 0.01 s.")
    p.add_argument("--band-energy-normalize-per-trace", action=argparse.BooleanOptionalAction, default=True,
                   help="Normalize band-energy curves per trace. Default: true.")
    p.add_argument("--band-energy-log10", action="store_true",
                   help="Plot log10 band energy instead of linear energy.")
...
def main(argv=None):
    ...
    band_energy_bands = parse_frequency_bands(args.band_energy_bands)
        ...
        try:
            ...
            if args.write_band_energy:
                plot_band_energy(
                    ...
                    receiver_x_m=rx,
                    source_x_m=src.x_m,
                    title=f"Band energy: real {rf.name}",
                    outfile=odir / "band_energy_real.png",
                    bands=band_energy_bands,
                ...
                plot_band_energy(
                    ...
                    receiver_x_m=rx,
                    source_x_m=src.x_m,
                    title="Band energy: synthetic no-void",
                    outfile=odir / "band_energy_synthetic_novoid.png",
                    bands=band_energy_bands,
                    window_s=args.band_energy_window_s,
                    step_s=args.band_energy_step_s,
                    normalize_per_trace=args.band_energy_normalize_per_trace,
                ...
                plot_band_energy(
                    ...
                    receiver_x_m=rx,
                    source_x_m=src.x_m,
                    title="Band energy: difference real - synthetic",
                    outfile=odir / "band_energy_difference_real_minus_synthetic.png",
                    bands=band_energy_bands,
                    window_s=args.band_energy_window_s,
                    step_s=args.band_energy_step_s,
                    normalize_per_trace=args.band_energy_normalize_per_trace,
                ...
                if args.write_trace_normalized_figures:
                    plot_band_energy(
                        ...
                        receiver_x_m=rx,
                        source_x_m=src.x_m,
                        title="Band energy: trace-normalized difference real - synthetic",
                        outfile=odir / "band_energy_difference_trace_normalized.png",
                        bands=band_energy_bands,
                        window_s=args.band_energy_window_s,
                        step_s=args.band_energy_step_s,
                        normalize_per_trace=args.band_energy_normalize_per_trace,
            ...
            if args.write_individual_wiggles:
                plot_wiggle(t, rx, rd, src.x_m, f"Real {rf.name}", odir / "wiggle_real.png", cave_extent=cave_extent)
                plot_wiggle(t, rx, sd, src.x_m, "Synthetic no-void", odir / "wiggle_synthetic_novoid.png", cave_extent=cave_extent)
                plot_wiggle(t, rx, diff, src.x_m, "Difference real - synthetic", odir / "wiggle_difference_real_minus_synthetic.png", cave_extent=cave_extent)

            norm_freq = not args.no_frequency_trace_normalization
            plot_freq(t, rx, rd, src.x_m, f"Frequency vs receiver: real {rf.name}", odir / "frequency_receiver_real.png", args.max_freq_hz, norm_freq, cave_extent=cave_extent)
            plot_freq(t, rx, sd, src.x_m, "Frequency vs receiver: synthetic no-void", odir / "frequency_receiver_synthetic_novoid.png", args.max_freq_hz, norm_freq, cave_extent=cave_extent)
            plot_freq(t, rx, diff, src.x_m, "Frequency vs receiver: difference real - synthetic", odir / "frequency_receiver_difference.png", args.max_freq_hz, norm_freq, cave_extent=cave_extent)

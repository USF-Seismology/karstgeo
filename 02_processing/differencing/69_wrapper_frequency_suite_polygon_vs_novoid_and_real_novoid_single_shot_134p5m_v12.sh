...
if [[ ! -f "$ENGINE" && -f "$SCRIPT_DIR/65_compare_gather_pairs_v16.py" ]]; then
  echo "WARNING: v17 engine not found; falling back to v16." >&2
  ENGINE="$SCRIPT_DIR/65_compare_gather_pairs_v16.py"
fi
...
OUT_ROOT="${OUT_ROOT:-$BASE/02_Modelling/Seismic/differencing/frequency_suite_polygon_vs_novoid_and_real_novoid_single_shot_134p5m_v12}"
MOVIE_ROOT="${MOVIE_ROOT:-$BASE/02_Modelling/Seismic/differencing/movies_frequency_suite_polygon_vs_novoid_and_real_novoid_single_shot_134p5m_v12}"
...
#
# IMPORTANT for macOS /bin/bash 3.2 compatibility and safety:
# do not let the whole frequency list become one array element.
# FREQS can be overridden as either:
#   FREQS="30 40 50" bash this_script.sh
# or:
#   FREQS="30,40,50" bash this_script.sh
FREQS_DEFAULT="30 40 50 60 70 80 100"
FREQS_TEXT="${FREQS:-$FREQS_DEFAULT}"
FREQS_TEXT="${FREQS_TEXT//,/ }"
FREQS=()
for _f in $FREQS_TEXT; do
  [[ -n "$_f" ]] && FREQS+=("$_f")
done
unset _f

SYN_COMPONENT_FILES_TEXT="${SYN_COMPONENT_FILES:-Ux_file_single_v.su Uz_file_single_v.su}"
SYN_COMPONENT_FILES=()
for _c in $SYN_COMPONENT_FILES_TEXT; do
  [[ -n "$_c" ]] && SYN_COMPONENT_FILES+=("$_c")
done
unset _c

REAL_COMPONENT_FILES_TEXT="${REAL_COMPONENT_FILES:-Uz_file_single_v.su}"
REAL_COMPONENT_FILES=()
for _c in $REAL_COMPONENT_FILES_TEXT; do
  [[ -n "$_c" ]] && REAL_COMPONENT_FILES+=("$_c")
done
unset _c
...
#
# This frequency-test suite only has synthetic source x=134.5 m.
# Therefore the default real-data run is intentionally restricted to the
# matching real shot only. With the T1 mapping 3005=82.5 m, dx=2 m,
# and duplicate 102.5 m files 3015/3016, x=134.5 m corresponds to file 3032.
# Override REAL_FIRST_FILE/REAL_LAST_FILE/REAL_SHOT_FIRST_X_M only if you have
# a synthetic frequency suite for additional shot positions.
...
REAL_FIRST_FILE="${REAL_FIRST_FILE:-3032}"
REAL_LAST_FILE="${REAL_LAST_FILE:-3032}"
REAL_SHOT_FIRST_X_M="${REAL_SHOT_FIRST_X_M:-134.5}"
REAL_SHOT_DX_M="${REAL_SHOT_DX_M:-2.0}"
REAL_SHOT_DUPLICATE_X_M="${REAL_SHOT_DUPLICATE_X_M:-102.5}"
REAL_SHOT_DUPLICATE_FILES="${REAL_SHOT_DUPLICATE_FILES:-3016}"
...
run_synthetic_frequency_component() {
  ...
  if [[ "$freq" == *" "* || "$freq" == *","* ]]; then
    echo "ERROR: internal frequency loop received a non-scalar frequency: '$freq'" >&2
    return 90
  fi
...
run_real_novoid_frequency_component() {
  ...
  if [[ "$freq" == *" "* || "$freq" == *","* ]]; then
    echo "ERROR: internal real-vs-synthetic loop received a non-scalar frequency: '$freq'" >&2
    return 91
  fi
  ...
  local args=(
    ...
    --output-dir "$outdir"
    --scale-mode "${REAL_SYN_SCALE_MODE:-trace_rms}"
    --scale-window-s "${REAL_SYN_SCALE_WINDOW_S:-0.02,0.20}"
    --scale-trim-percentile "${REAL_SYN_SCALE_TRIM_PERCENTILE:-99}"
    --scale-clip-min "${REAL_SYN_SCALE_CLIP_MIN:-0.1}"
    --scale-clip-max "${REAL_SYN_SCALE_CLIP_MAX:-100}"
...
echo "WRITE_FK:         $WRITE_FK  (vmin=500 m/s; split-at-source; f-k SEG-Y enabled)"
echo "SEG-Y:           processed gather SEG-Y enabled"
echo "REAL_DIR:         ${REAL_DIR:-<unset>}"
echo "REAL files:       ${REAL_FIRST_FILE}-${REAL_LAST_FILE}, first x=${REAL_SHOT_FIRST_X_M} m"

...
# 65_compare_gather_pairs_v18.py.
#
...
#   ENGINE=/path/to/65_compare_gather_pairs_v18.py
...
ENGINE="${ENGINE:-$SCRIPT_DIR/65_compare_gather_pairs_v18.py}"
...
OUT_BASE="${OUT_BASE:-$BASE/02_Modelling/Seismic/differencing_final}"
MOVIE_BASE="${MOVIE_BASE:-$BASE/02_Modelling/Seismic/differencing_final}"
...
build_product_args() {
  ...
  if [[ "$WRITE_FK" == "1" ]]; then
    COMMON_ARGS+=(
      ...
      --fk-use-taper
      --no-fk-split-at-source
      --fk-spatial-taper-fraction "${FK_SPATIAL_TAPER_FRACTION:-0.0}"
      --fk-pad-factor "${FK_PAD_FACTOR:-1}"
    )
  fi
}
...
# ----------------------------------------------------------------------------
run_t1() {
  local out_root="${OUT_ROOT:-$OUT_BASE/comparison_products_final}"
  local movie_root="${MOVIE_ROOT:-$MOVIE_BASE/movies_final}"
  local cave_model="${CAVE_MODEL:-$SOURCES_GROUNDED/9_LAYER_MODEL_TOPO_POLYGON_CENTERED_135m_1m_50Hz_DX_DZ_0d5m_DT_1e-5s}"
  local no_void_model="${NO_VOID_MODEL:-$SOURCES_GROUNDED/9_LAYER_MODEL_TOPO_NO_VOID_1m_50Hz_DX_DZ_0d5m_DT_1e-5s}"

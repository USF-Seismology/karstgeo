...
def _read_header_geometry(header, i: int) -> Tuple[float, float]:
    ...
    # Try the standard ObsPy SEG-Y names first, then common SU aliases.
    gx = None
...
def _read_su_geometry(tr, i: int) -> Tuple[float, float]:
    """Return receiver_x_m, source_x_m from Seismic Unix trace header."""
...
def _read_mseed_geometry(tr, i: int, station_scale: float = 1.0) -> Tuple[float, float]:
    """
    Best-effort receiver geometry for MiniSEED/SU/SEG-2-like traces.

    The database geometry layer may attach stats.receiver_x_m and
    stats.source_x_m before this function is called. Those take priority.
    """
...
def _pick_col(cols: List[str], names: List[str]) -> Optional[str]:
...
def debug_database_schema(db_path: Path) -> None:
    ...
    try:
        ...
        for t in tables:
            ...
            try:
                n = conn.execute(f"SELECT COUNT(*) FROM {_quote_ident(t)}").fetchone()[0]
            except Exception:
                n = "?"
            print(f"\n{t} ({n} rows)")
            print("  columns:", cols)
            try:
                sample = conn.execute(f"SELECT * FROM {_quote_ident(t)} LIMIT 3").fetchall()
                print("  sample:", sample)
...
def _path_match_clause(cols: List[str], path: Path, alias: str = "") -> Tuple[str, List[str]]:
    """Return SQL WHERE fragment and parameters for likely path/filename columns."""
    prefix = f"{alias}." if alias else ""
    candidates = []
    for c in cols:
        cl = c.lower()
        if any(token in cl for token in ("path", "file", "filename", "basename")):
            candidates.append(c)
    if not candidates:
        return "", []
    vals = [str(path), path.name, path.stem]
    clauses = []
    params: List[str] = []
    for c in candidates:
        qc = prefix + _quote_ident(c)
        clauses.append(f"({qc} = ? OR {qc} = ? OR {qc} = ? OR {qc} LIKE ?)")
        params.extend([str(path), path.name, path.stem, f"%{path.name}%"])
    return " OR ".join(clauses), params


def load_db_geometry_for_file(db_path: Path, waveform_path: Path, ntraces: int) -> Tuple[Dict[int, Tuple[float, float]], str]:
    """
    Try to recover trace_index -> (receiver_x_m, source_x_m) from the project SQLite DB.

    This is intentionally schema-tolerant. It first looks for the expected
    shot_gather_files + trace_index relationship, then falls back to any table
    with trace-index and receiver-x columns that can be matched by filename.
    """
    ...
    geom: Dict[int, Tuple[float, float]] = {}
    notes: List[str] = []

    try:
        tables = [r[0] for r in conn.execute("SELECT name FROM sqlite_master WHERE type='table'")]

        # Preferred schema: shot_gather_files identifies the loaded waveform;
        # trace_index has per-trace receiver positions and often source positions.
        if "shot_gather_files" in tables and "trace_index" in tables:
            sg_cols = _table_columns(conn, "shot_gather_files")
            ti_cols = _table_columns(conn, "trace_index")
            sg_id_col = _pick_col(sg_cols, ["id", "shot_gather_file_id", "file_id"])
            ti_file_col = _pick_col(ti_cols, ["shot_gather_file_id", "file_id", "gather_file_id"])
            ti_idx_col = _pick_col(ti_cols, ["trace_index", "trace_number", "trace_id_in_file", "itrace", "i"])
            ti_rx_col = _pick_col(ti_cols, ["receiver_x_m", "receiver_x", "x_m", "rx_m", "x"])
            ti_sx_col = _pick_col(ti_cols, ["source_x_m", "source_x", "shot_x_m", "sx_m"])
            match_sql, params = _path_match_clause(sg_cols, waveform_path, alias="sg")
            if sg_id_col and ti_file_col and ti_idx_col and ti_rx_col and match_sql:
                sx_expr = f"ti.{_quote_ident(ti_sx_col)}" if ti_sx_col else "NULL"
                sql = (
                    f"SELECT ti.{_quote_ident(ti_idx_col)} AS trace_index, "
                    f"ti.{_quote_ident(ti_rx_col)} AS receiver_x_m, {sx_expr} AS source_x_m "
                    f"FROM shot_gather_files sg JOIN trace_index ti "
                    f"ON sg.{_quote_ident(sg_id_col)} = ti.{_quote_ident(ti_file_col)} "
                    f"WHERE {match_sql} ORDER BY trace_index"
                )
                rows = conn.execute(sql, params).fetchall()
                for r in rows:
                    try:
                        idx = int(r["trace_index"])
                        if 0 <= idx < max(ntraces, idx + 1):
                            rx = float(r["receiver_x_m"])
                            sx = float(r["source_x_m"]) if r["source_x_m"] is not None else np.nan
                            geom[idx] = (rx, sx)
                    except Exception:
                        continue
                if geom:
                    notes.append(f"shot_gather_files→trace_index matched {len(geom)} traces")
                    return geom, "; ".join(notes)

        # Direct fallback: a table such as trace_index with filename/path columns.
        for table in tables:
            cols = _table_columns(conn, table)
            idx_col = _pick_col(cols, ["trace_index", "trace_number", "trace_id_in_file", "itrace", "i"])
            rx_col = _pick_col(cols, ["receiver_x_m", "receiver_x", "x_m", "rx_m", "x"])
            sx_col = _pick_col(cols, ["source_x_m", "source_x", "shot_x_m", "sx_m"])
            match_sql, params = _path_match_clause(cols, waveform_path)
            if not (idx_col and rx_col and match_sql):
                continue
            sx_expr = _quote_ident(sx_col) if sx_col else "NULL"
            sql = (
                f"SELECT {_quote_ident(idx_col)} AS trace_index, "
                f"{_quote_ident(rx_col)} AS receiver_x_m, {sx_expr} AS source_x_m "
                f"FROM {_quote_ident(table)} WHERE {match_sql} ORDER BY trace_index"
            )
            try:
                rows = conn.execute(sql, params).fetchall()
            except Exception:
                continue
            tmp: Dict[int, Tuple[float, float]] = {}
            for r in rows:
                try:
                    idx = int(r["trace_index"])
                    rx = float(r["receiver_x_m"])
            ...
            if tmp:
                geom.update(tmp)
                notes.append(f"{table} matched {len(tmp)} traces")
                return geom, "; ".join(notes)

        # Last fallback: receiver_geometry table in row order, but only if its
        # row count is plausible for this gather. This is less safe, so it is last.
        if "receiver_geometry" in tables:
            cols = _table_columns(conn, "receiver_geometry")
            rx_col = _pick_col(cols, ["receiver_x_m", "receiver_x", "x_m", "rx_m", "x"])
            if rx_col:
                sx_col = _pick_col(cols, ["source_x_m", "source_x", "shot_x_m", "sx_m"])
                sx_expr = _quote_ident(sx_col) if sx_col else "NULL"
                sql = f"SELECT {_quote_ident(rx_col)} AS receiver_x_m, {sx_expr} AS source_x_m FROM receiver_geometry ORDER BY 1"
                ...
                if len(rows) == ntraces:
                    for idx, r in enumerate(rows):
                        ...
                        sx = float(r["source_x_m"]) if r["source_x_m"] is not None else np.nan
                        geom[idx] = (rx, sx)
                    notes.append(f"receiver_geometry row-order fallback matched {len(geom)} traces")
                    return geom, "; ".join(notes)

        return {}, "no matching geometry rows found"
    finally:
        conn.close()

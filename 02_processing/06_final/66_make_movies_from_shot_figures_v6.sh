...
# New in v6:\n#   skip macOS AppleDouble files and validate PNG signatures\n#\n# Retained from v5:
...
# Avoid Path.rglob() here. On large Box/cloud trees it can spend several minutes
# traversing irrelevant output and hidden directories without printing anything.
# os.walk() lets us prune aggressively and report progress.
PRUNE_DIR_NAMES = {
    "_movies",
    "_frames_tmp",
    "movies",
...
candidate_images = 0
accepted_images = 0
skipped_appledouble = 0
skipped_invalid_images = 0

PNG_SIGNATURE = b"\x89PNG\r\n\x1a\n"

print("Scanning PNG tree...", flush=True)

for current_root, dirnames, filenames in os.walk(root):
    # Modify dirnames in place so os.walk does not descend into these trees.
    dirnames[:] = sorted(
        [
            name
            for name in dirnames
            if (
                name not in PRUNE_DIR_NAMES
                and not name.startswith(PRUNE_DIR_PREFIXES)
    ...
    if directories_scanned == 1 or directories_scanned % 100 == 0:
        print(
            f"  scanned {directories_scanned:,} directories; "
            f"found {candidate_images:,} *.{ext} files; "
    ...
    for filename in filenames:
        ...
        # macOS AppleDouble/resource-fork sidecars can have names such as
        # ._figure.png. They are metadata files, not images.
        ...
        # Defensively verify PNG contents. This also excludes truncated files
        # and unrelated files that merely carry a .png extension.
        if ext.lower() == "png":
            try:
            ...
            if signature != PNG_SIGNATURE:
                skipped_invalid_images += 1
                print(f"WARNING: skipping invalid PNG: {path}", flush=True)
                continue
        ...
        if require_source_x and source_x_m is None:
            # Do not print thousands of warnings while scanning miscellaneous
            # PNGs. They are counted and summarized below.
            continue
...
print(
    f"Scan complete: {directories_scanned:,} directories, "
    f"{candidate_images:,} candidate images, "
    f"{accepted_images:,} accepted frames, "
    f"{skipped_appledouble:,} AppleDouble files skipped, "
    f"{skipped_invalid_images:,} invalid images skipped.",
    flush=True,
)
